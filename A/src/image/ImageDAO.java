...
public class ImageDAO {
	...
	public boolean checkRefno(int no){
		...
		sql.append(" select count(refno) from Aimage ");
	...
	public boolean createReply(ImageDTO dto){
		...
		sql.append(" INSERT INTO Aimage(no, name, title, content, passwd, wdate, gno, ino, ano,refno,  filename) ");
		sql.append(" VALUES((SELECT NVL(MAX(no), 0) + 1 FROM Aimage), ");
		sql.append(" ?, ?, ?, ?,  sysdate, ");
		sql.append(" ?, ?, ?, ? , ? )  ");
	...
	public boolean passwd(Map map){
		...
		ResultSet rs= null;
		sql.append(" select count(no) from Aimage ");
	...
	public boolean create(ImageDTO dto) {
		...
		sql.append(" insert into Aimage ( no , name , title , content ,passwd  ,wdate ,filename ,gno  ) ");
		sql.append(
				" values((select nvl(max(no),0)+1 from Aimage) , ? , ? , ? , ? , sysdate, ?,(select nvl(max(gno),0)+1 from image) )");
